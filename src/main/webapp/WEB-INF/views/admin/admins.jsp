...
<header class="header--form-page">
        ...
        <table id="table" class="display table table-striped" style="width:100%">
            ...
            <c:forEach items="${admins}" var="admin">
                <tr>
                    ...
                    <td><c:out value="${admin.active}"/></td>
                    <td>
                        <c:if test="${admin.username != username}">
                        <button type="button" class="btn btn-outline-danger btn-sm" data-toggle="modal"
                                data-target="#adminDelete" data-name="${admin.id}">
                            USUŃ
                        </button>
                        </c:if>
                        <c:if test="${admin.username == username}">
                            <button disabled type="button" class="btn btn-outline-secondary btn-sm">USUŃ</button>
                        </c:if>
                        <c:if test="${admin.username != username}">
                        <a href="/admin/Astatus/${admin.id}" class="btn btn-outline-warning btn-sm">ZMIEŃ STATUS</a>
                        </c:if>
                        <c:if test="${admin.username == username}">
                            <button disabled type="button" class="btn btn-outline-secondary btn-sm">ZMIEŃ STATUS</button>
                        </c:if>
                        <a href="/admin/Aedit/${admin.id}" class="btn btn-outline-success btn-sm">EDYTUJ</a></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
        <!-- Modal -->
        <div class="modal fade" id="adminDelete" tabindex="-1" aria-labelledby="exampleModalLabel"
             aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p> Uwaga! Nie ma możliwości cofnięcia usuwania administratora!</p>
                        <p> Po kliknięciu usuń administrator zostanie usunięty z bazy danych na stałe.</p>
                        <p> Jeśli chcesz zablokować administratora nie usuwając go, zmień jego status na false.</p>
                    </div>
    ...
    <div class="row">
        <div class="container p-3 my-3 border shadow p-1 mb-1 bg-white rounded">
            <a href="/admin/registerA" style="float:left" class="btn btn-success" >Dodaj administratora</a>
        </div>
    </div>
</header>
